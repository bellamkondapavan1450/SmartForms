...
public class StarredFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        databaseReference = firebaseDatabase.getReference().child("Users").child(auth.getCurrentUser().getUid()).child("Starred");
        ...
        if (bundle != null) {
            ...
            query = FirebaseDatabase.getInstance().getReference().child("data")
                    .orderByChild("sender").startAt(s).endAt(s + "\uf8ff");
        } else
                ...
                holder.setMailItemClickListener(new MailItemClickListener() {
                            ...
                            dataref.child("Inbox").addValueEventListener(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (snapshot.hasChild(ClickedItemKey))
                                        dataref.child("Inbox").child(ClickedItemKey).child("isRead").setValue(true);
                                    else
                                        dataref.child("Sent").child(ClickedItemKey).child("isRead").setValue(true);
                                }
                    ...
                    public void onStarClick(View view, int position) {
                        ...
                        dataref.child("Inbox").addValueEventListener(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                if (snapshot.hasChild(ClickedItemKey))
                                    dataref.child("Inbox").child(ClickedItemKey).child("isImportant").setValue(false);
                                else
                                    dataref.child("Sent").child(ClickedItemKey).child("isImportant").setValue(false);
                            }
